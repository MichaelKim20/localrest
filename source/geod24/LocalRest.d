...
public class RemoteAPI (API) : API
{
    ...
    ***************************************************************************/
    ...
    /***************************************************************************
        ...
        Params:
            cmd    = the request to run (contains the method name and the arguments)
            ...
            static foreach (ovrld; __traits(getOverloads, API, member))
            {
                ...
                q{
                    ...
                    try
                    {
                        ...
                        {
                            cmd.sender.send(Response(Status.Success, cmd.id, node.%1$s(args.args).serializeToJsonString()));
                        }
                        else
                        {
                            node.%1$s(args.args);
            ...
            mixin(q{
                override ReturnType!(ovrld) } ~ member ~ q{ (Parameters!ovrld params)
                {
                    ...
                    auto res = () @trusted
                    {
                        ...
                        {
                            cmd = Command(thisLocalTransceiver, thisLocalWaitingManager.getNextResponseId(), ovrld.mangleof, serialized);
                            this._transceiver.send(cmd);

                            res = thisLocalWaitingManager.waitResponse(cmd.id, this._timeout);
                        }
...
unittest
{
    ...
    static class Node : API
    {
        ...
        public override void  asyncCall () { runTask(() => assertThrown!Exception(cast(void)this.remote.call)); }
        size_t count;
