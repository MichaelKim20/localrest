...
{
    this (Command msg) { this.cmd = msg; this.tag = MessageType.request; }
...
private class LocalTransceiver : Transceiver
{
    ...
    private bool _belongToNode;


    /// Ctor
    public this (bool value) @safe nothrow
    {
        this.chan = new Channel!Message(64*1024);
        this._belongToNode = value;
    }

    public @property bool belongToNode () @safe @nogc pure
    {
        return this._belongToNode;
    }
...
{
    assert(thisScheduler !is null, "Cannot call this function from the main thread");
    scope c = thisScheduler.newCondition();
    c.wait(timeout);
}
...
public class RemoteAPI (API) : API
{
    ...
    ***************************************************************************/
    ...
    {
        auto childTransceiver = spawned!Impl(args);
        return new RemoteAPI(childTransceiver, timeout);
    }
    ...
    ***************************************************************************/
    ...
    private static LocalTransceiver spawned (Implementation) (CtorParams!Implementation cargs)
    {
        ...
        thread_scheduler.spawn({
            ...
            thisScheduler.start({
                thisLocalTransceiver = new LocalTransceiver(true);
                ...
                while (!terminate)
                {
                    ...
                    if (!isSleeping())
                    {
                        ...
                        {
                            await_req.each!(cmd => handleCmd(cmd));
                            await_req.length = 0;
    ...
    private LocalTransceiver childTransceiver;
    ...
    ***************************************************************************/
    ...
    {
        this.childTransceiver = cast(LocalTransceiver)transceiver;
        this.owner = isOwner;
    ...
    ***************************************************************************/
    ...
    private mixin template ControlInterface ()
    {
        ...
        ***********************************************************************/
        ...
        public LocalTransceiver transceiver () @nogc pure nothrow
        {
        ...
        ***********************************************************************/
        ...
        {
            this.childTransceiver.send(ShutdownCommand());
        }
        ...
        ***********************************************************************/
        ...
        public void filter (alias method, OverloadParams...) () @trusted
        {
            ...
            this.childTransceiver.send(FilterAPI(mangled, pretty));
        }
        ...
        ***********************************************************************/
        ...
        {
            this.childTransceiver.send(FilterAPI(""));
        }
    }
            ...
            mixin(q{
                override ReturnType!(ovrld) } ~ member ~ q{ (Parameters!ovrld params)
                {
                    auto res = () @trusted {
                        ...
                        // from Node to Node
                        if ((thisLocalTransceiver !is null) && (thisLocalTransceiver.belongToNode))
                        {
                            command = Command(thisLocalTransceiver, thisLocalWaitingManager.getNextResponseId(), ovrld.mangleof, serialized);
                            this.childTransceiver.send(command);

                            res = thisLocalWaitingManager.waitResponse(command.id, this.timeout);
                        }
                        ...
                        else
                        {
                            ...
                            if (thisLocalTransceiver is null)
                                thisLocalTransceiver = new LocalTransceiver(false);
...
unittest
{
    ...
    auto node_1 = RemoteAPI!API.spawn!Node();
    auto node_2 = RemoteAPI!API.spawn!Node();
    node_transceiver = node_2.transceiver();
    node_1.check();
    Thread.sleep(3000.msecs);
    node_1.ctrl.shutdown();
    node_2.ctrl.shutdown();
